...
logger = setup_logging("c_to_n.py")

from sklearn.preprocessing import OneHotEncoder
from sklearn.preprocessing import OrdinalEncoder

def categorical_to_numeric(x_train_cat,x_test_cat):
    try:
        ...
        #Nominal Encoding
        logger.info("-----------------Nominal Encoding (Gender, Region)-------------------")
        one_hot_encoder = OneHotEncoder(drop='first')
        one_hot_encoder.fit(x_train_cat[['Gender', 'Region']])
        logger.info(f"{one_hot_encoder.get_feature_names_out()}")
        result = one_hot_encoder.transform(x_train_cat[['Gender', 'Region']]).toarray()
        ...
        result1 = one_hot_encoder.transform(x_test_cat[['Gender', 'Region']]).toarray()
        ...
        #Ordinal Encoding
        logger.info("-----------------Ordinal Encoding ('Rented_OwnHouse', 'Occupation', 'Education')------------------")
        ordinal_encoder = OrdinalEncoder()
        ordinal_encoder.fit(x_train_cat[['Rented_OwnHouse', 'Occupation', 'Education']])
        logger.info(f" features names : {ordinal_encoder.get_feature_names_out()}")
        result2 = ordinal_encoder.transform(x_train_cat[['Rented_OwnHouse', 'Occupation', 'Education']])
        ...
        result3 = ordinal_encoder.transform(x_test_cat[['Rented_OwnHouse', 'Occupation', 'Education']])
        ...
        logger.info(f"{x_train_cat["Education_ordinal"].unique()}")
        logger.info(f"{x_train_cat["Rented_OwnHouse_ordinal"].unique()}")
        logger.info(f"{x_train_cat["Occupation_ordinal"].unique()}")
