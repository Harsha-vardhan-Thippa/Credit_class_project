...
logger = setup_logging("feature_selection.py")
...
def complete_feature_selection(x_train_num,x_test_num,y_train) :
    try:
        ...
        logger.info(f"columns to remove (quesi constant)(un used columns): {x_train_fs.columns[~reg_quesi.get_support()]}")
        ...
        # plt.show()
        x_train_fs_1 = x_train_fs_1.drop(["DebtRatio_yeo_trim"],axis=1)
        x_test_fs_2 = x_test_fs_2.drop(["DebtRatio_yeo_trim"],axis=1)
        ...
        logger.info("Failed to complete feature selection process")
        logger.error(e)
