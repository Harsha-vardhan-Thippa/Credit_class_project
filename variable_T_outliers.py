...
logger = setup_logging("variable_T_outliers.py")
    ...
    try:
        ...
        for i in x_train_num.columns:
            ...
            x_train_num[i+"_yeo_trim"] = np.where(x_train_num[i+"_yeo"]>upperlimit,upperlimit,
                     np.where(x_train_num[i+"_yeo"]<lowerlimit,lowerlimit,x_train_num[i+"_yeo"]))
            x_train_num = x_train_num.drop([i+"_yeo"], axis=1)

            x_test_num[i+"_yeo_trim"] = np.where(x_test_num[i]>upperlimit,upperlimit,np.where(x_test_num[i]<lowerlimit,lowerlimit,x_test_num[i]))
